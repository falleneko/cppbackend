...
model::Game LoadGame(const std::filesystem::path& json_path) {
    ...
    const double default_dog_speed = config.contains("defaultDogSpeed")
        ? json::value_to<double>(config.at("defaultDogSpeed"))
        : 1.0;

    for (const auto& val : config.at("maps").as_array()) {
        auto obj = val.as_object();
        model::Map::Id id{json::value_to<std::string>(obj.at("id"))};
        model::Map map{
            id,
            json::value_to<std::string>(obj.at("name")),
            obj.contains("dogSpeed")
                ? json::value_to<double>(obj.at("dogSpeed"))
                : default_dog_speed
        };
